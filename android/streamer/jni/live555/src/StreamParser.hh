...
protected: // we're a virtual base class
  ...
  u_int32_t test4Bytes() { // as above, but doesn't advance ptr
	//__android_log_write(ANDROID_LOG_DEBUG, "streamer", "test4Bytes() - begin");
    ensureValidBytes(4);
    if(LIVE555_EXCEPTION_FLAG)
    	return 0;
    //__android_log_write(ANDROID_LOG_DEBUG, "streamer", "test4Bytes() - middle");

    unsigned char const* ptr = nextToParse();
    //__android_log_write(ANDROID_LOG_DEBUG, "streamer", "test4Bytes() - end");
