...
extern int LIVE555_EXCEPTION_FLAG;
...
//MODIFIED How should this be handled?
unsigned MPEG4VideoStreamParser::parse()
{
	//__android_log_write(ANDROID_LOG_DEBUG, "streamer", "MPEG4VideoStreamParser::parse()");
	//try
	//{
		unsigned i;
		...
		/*char str[100];
		sprintf(str, "LIVE555_EXCEPTION_FLAG: %i", LIVE555_EXCEPTION_FLAG);
		__android_log_write(ANDROID_LOG_DEBUG, "streamer", str);*/

		if(LIVE555_EXCEPTION_FLAG)
		{
			LIVE555_EXCEPTION_FLAG = 0;
			__android_log_write(ANDROID_LOG_DEBUG, "streamer",
					"MPEG4VideoStreamParser::parse() EXCEPTION (This is normal behavior - *not* an error)");
...
::parseVisualObjectSequence(Boolean haveSeenStartCode) {
	char str[100];
#ifdef DEBUG
	__android_log_write(ANDROID_LOG_DEBUG, "streamer", "parsing VisualObjectSequence");
#endif
  ...
  if (!haveSeenStartCode) {
    while ((first4Bytes = test4Bytes()) != VISUAL_OBJECT_SEQUENCE_START_CODE) {
    	...
    	if(LIVE555_EXCEPTION_FLAG)
    		return 0;
#ifdef DEBUG
    sprintf(str, "ignoring non VS header: 0x%08x\n", first4Bytes);
    __android_log_write(ANDROID_LOG_DEBUG, "streamer", str);
#endif
...
#ifdef DEBUG
  sprintf(str, "profile_and_level_indication: %02x\n", pali);
  __android_log_write(ANDROID_LOG_DEBUG, "streamer", str);
#endif
  saveByte(pali);
...
unsigned MPEG4VideoStreamParser::parseVisualObject() {
	//__android_log_write(ANDROID_LOG_DEBUG, "streamer", "MPEG4VideoStreamParser::parseVisualObject()");
#ifdef DEBUG
...
unsigned MPEG4VideoStreamParser::parseVideoObjectLayer() {
	//__android_log_write(ANDROID_LOG_DEBUG, "streamer", "MPEG4VideoStreamParser::parseVideoObjectLayer()");
#ifdef DEBUG
...
unsigned MPEG4VideoStreamParser::parseGroupOfVideoObjectPlane() {
	//__android_log_write(ANDROID_LOG_DEBUG, "streamer", "MPEG4VideoStreamParser::parseGroupOfVideoObjectPlane()");
#ifdef DEBUG
...
unsigned MPEG4VideoStreamParser::parseVideoObjectPlane() {
	//__android_log_write(ANDROID_LOG_DEBUG, "streamer", "MPEG4VideoStreamParser::parseVideoObjectPlane()");
#ifdef DEBUG
...
unsigned MPEG4VideoStreamParser::parseVisualObjectSequenceEndCode() {
	//__android_log_write(ANDROID_LOG_DEBUG, "streamer", "MPEG4VideoStreamParser::parseVisualObjectSequenceEndCode()");
#ifdef DEBUG
